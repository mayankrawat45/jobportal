...
export const createJob = async (req,res)=>{
    try {
        ...
        // handle arrrays if sent as JSON string
...
export const getAllJobs = async (req,res) => {
    try {
        ...
        // search by roleName, companyName, techStack
        if(search){
            query.$or = [
        ...
        if(jobType) {
            ...
            const normalizeTypeToReagex = (type) => {
            ...
            query.jobType = { $in: types.map(normalizeTypeToReagex) };
        };
...
export const getJobsByAdmin = async (req,res) => { 
    try {
        ...
        console.error("Error fetching the admin jobs:", error);
        return res.status(500).json({
            success: false,
            message: "Server error"
        })
...
export const updateJob = async (req,res) => {
    try {
        ...
        if(req.file){
            const uploadRes = await uploadToCloudinary(req.file.buffer, "jobportal/logos", "image", req.file.originalname);
            companyLogo = uploadRes.secure_url;
        } //update image will get saved in cloudinary logos folder
