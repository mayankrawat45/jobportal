...
const CompanyPage = () => {
    ...
    // to save or unsave the questions
    ...
    return (
        <div className={s.container}>
            <div className={s.innerContainer}>
                ...
                <div className={s.tabsContainer}>
                    <div className={s.tabsGrid}>
                        {companies.map((company) => {
                            ...
                            return (
                                <button key={company._id} onClick={() => {
                                    setSelectedCompany(company._id);
                                    setShowAllQuestions(false);
                                }} className={s.tabButton}>
                                    <div className={s.tabImageWrapper}>
                                        <div className={s.tabImageContainer}>
                                            <img src={company.logo} alt={company.companyName} className={s.tabImage} />
                                        </div>
                                    </div>
                                    <span className={`${s.tabText} ${isSelected ? s.tabTextSelected : s.tabTextUnselected}`} style={{ maxWidth: 140 }}>
                                        {company.companyName}
                                    </span>
                                </button>
                            )
                ...
                <div className={s.questionsHeader}>
                    <h2 className={s.questionTitle}>
                        {selectedCompanyData ? `${selectedCompanyData.companyName} Interview Questions` : "Interview Questions"}
            ...
            {toast.show && (
                <Toast type={toast.type} message={toast.message} onClose={()=>setToast({
                    ...toast,
                    show:false
                })}/>
            )}
