...
import { AlertCircle, Briefcase, Calendar, CheckCircle, Filter, Hash, Image as ImageIcon, Loader, Loader2, Upload } from 'lucide-react'
import axios from 'axios';


// HELPERS
// it will parse the csv data into the plain text
...
const splitMulti = (cell) =>
    (cell || "")
        ...
        .split(/[\n;\|]+/)
        .map((s) => s.trim())
        .filter(Boolean); //will allow to split by / and return cell
...
const Toast = ({ message, type, onClose }) => {
    ...
    const bg = type === "success" ? "bg-green-50 border-green-500" : "bg-red-50 border-red-500";
    const Icon = type === "success" ? CheckCircle : AlertCircle;
    const iconColor = type === "success" ? "text-green-500" : "text-red-500";

    return (
        <div className={`fixed top-4 right-4 z-50 flex items-center gap-3 p-4 border-l-4 rounded-lg shadow-lg ${bg} animate-fadeInUp`}>
...
const RoleQuestionPage = () => {
    ...
    const handleAddRoleQuestion = async () => {
        ...
        try {
            ...
            console.error("Error Adding the role questions:", err);
            setToast({
    ...
    const handleImageChange = (e) => {
        const f = e.target.value && e.target.files[0];
            ...
            <div className={s.contentWrapper} >
                ...
                <div className={s.formGrid}>
                    <div className={s.formColumn}>
                        ...
                        <div className={s.card}>
                            ...
                            <div className={s.dropzone} onClick={()=> imageInputRef.current.click()}>
                              {imagePreview ? (
                                <img src={imagePreview} alt="logo" className={s.imagePreview} />
                              ): (
                                <>
                    ...
                    <div className={s.formColumn}>
                       <div className={s.card}>
                           <label className={s.cardLabel}>
                            <Filter className={s.labelIcon} /> Upload CSV{" "} 
                            <span className={s.requiredStar}>*</span>
                           </label>     
                            ...
                            {csvFile ?(
                                ...
                                <>
                                    ...
                                    <p className={s.uploadHint}>
                                            ...
                                            Company may include date: `Google (2021-03-10)`
                                    </p>
                                </>
                            )}

                            <input type="file" ref={csvInputRef} accept='.csv' onChange={handleCSVUpload}
                ...
                <div className={s.buttonContainer}>
                    <button onClick={handleAddRoleQuestion} disabled={isUploading} className={`${s.addButton} ${
                ...
                {questions.length > 0 && (
                    <div className={s.questionsSection}>
                        <h2 className={s.setImagePreview}>Loaded Questions ({questions.length})</h2>
                        <div className={s.questionsGrid}>
                            {questions.map((q, index)=> (
                                <div key={q.id} className={`${s.cardAnimation} ${
                                s.questionCard 
                                }`} style={{
