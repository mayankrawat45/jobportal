...
import { Briefcase, Building2, Calendar, Code, DollarSign, FileText, GraduationCap, Image as ImageIcon, ListChecks, Loader2, MapPin, Plus, Tag, Trash2, Upload, User, X } from 'lucide-react';
...
const Toast = ({ message, type, onClose }) => {
    ...
    return (
        <div className={s.toastWrapper}>
            <div className={`${s.toastContent} ${type === "sucess" ? s.toastSuccess : s.toastError
                }`}>
                <div className='relative'>
                    <div className={`${s.toastDot} ${type === "success" ? s.toastDotSuccess : s.toastDotError
                        }`}></div>
                    <div className={`${s.toastDotStatic} ${type === "success" ? s.toastDotSuccess : s.toastDotError
                        }`}></div>
                </div>
                ...
                >
                    <span className={s.fieldIconSpan}>
                        {Icon && (
                            <Icon
                                size={18}
                                className={`transition-colors duration-300 ${focused ? s.fieldIconFocused : ""
                                    }`}
                            />
                        )}
                    </span>
...
const ImageUpload = ({ image, setImage, error }) => {
    ...
    return (
        <div className={s.uploadContainer}>
            ...
            <div
                className={`${s.uploadDropzone} ${dragActive
...
const AddJobsPage = () => {
    ...
    }; // it will validate and then if not filled by the user it will show errors according to that
        ...
        <div className={s.pageContainer}>
            ...
            <div className={s.contentWrapper}>
                <div className={s.headerCenter}>
                    ...
                    <p className={s.subtitle}>Create a beautiful, high-converting job listing.</p>
                </div>
                <form onSubmit={handleSubmit} className={s.formCard} >
                    <div className={s.grid3}>
                        ...
                        <div className={s.mdColSpan2}>
                            <AnimatedField icon={Briefcase} label="Role Name" name="roleName"
                                value={formData.roleName} onChange={(e) => setFormData({
                                    ...formData,
                                    roleName: e.target.value
                                })}
                                error={errors.roleName} placeholder="e.g. Senior Frontend Developer"
                                required />

                            <AnimatedField icon={Building2} label="Company Name" name="companyName"
                                value={formData.companyName} onChange={(e) => setFormData({
                                    ...formData,
                                    companyName: e.target.value
                                })}
                                error={errors.companyName} placeholder="e.g. ACME Inc"
                                required />
                        </div>
                    </div>
                    {/* tech stack */}
                    <div className={s.arraySection}>
                        <label className={s.arrayLabel}>
                            <Code size={16} className={s.uploadIcon} /> Tech Stack{" "}
                            <span className={s.uploadRequired}>*</span>
                        </label>
                        {formData.techStack.map((tech, index) => (
                            <div key={index} className={s.arrayItemRow}>
                                ...
                                {formData.techStack.length > 1 && (
                                    <button type='button' onClick={() => removeArrayField("techStack", index)}
                        ...
                        {/* to add more techStacks */}
                        <button type='button' onClick={() => addArrayField("techStack")}
                            className={s.addBtn}>
                    ...
                    <div className={s.grid3}>
                        <AnimatedField icon={MapPin} label="location" name="location"
                            value={formData.location} onChange={(e) => setFormData({
                                ...formData,
                                location: e.target.value
                            })}
                            error={errors.location} placeholder="e.g. New York, NYr"
                            required />
                        <AnimatedField icon={Briefcase} label="Experience (years)" name="experience"
                            value={formData.experience} onChange={(e) => setFormData({
                                ...formData,
                                experience: e.target.value
                            })}
                            error={errors.experience} placeholder="3+ years"
                            required />
                        ...
                        <div className={s.salaryContainer}>
                            ...
                            <div className={s.salaryInputWrapper}>
                                <div
                                    className={`${s.salaryInputGroup} ${errors.salary
                    ...
                    <div className={s.grid3}>
                        <AnimatedField icon={Briefcase} label="Job type" name="jobtype"
                            type='select'
                        ...
                        <AnimatedField icon={Calendar} label="Post Date (Automatically set to Today)" name="postDate"
                            type='date'
                            value={formData.postDate} onChange={(e) => setFormData({
                                ...formData,
                        ...
                        <AnimatedField icon={Tag} label="Category" name="category"
                            type='select'
                    ...
                    <div className={s.grid2}>
                        ...
                        <AnimatedField
                            ...
                            placeholder="Brief decription of the role..."
                            required
                        />
                    </div>
                    {/* Responsibilities */}
                        ...
                        {formData.responsibilities.map((resp, index) => (
                            <div key={index} className={s.arrayItemRow}>
                                <input
                                    ...
                                    className={`${s.arrayInput} ${errors.responsibilities && !resp.trim()
                                        ? s.arrayInputError
                                        : s.arrayInputDefault
                                        }`}
                        ...
                        {formData.jobCriteria.map((crit, index) => (
                            <div key={index} className={s.arrayItemRow}>
                                <input
                                    ...
                                    className={`${s.arrayInput} ${errors.jobCriteria && !crit.trim()
                                        ? s.arrayInputError
                                        : s.arrayInputDefault
                                        }`}
                        ...
                        {formData.education.map((edu, index) => (
                            <div key={index} className={s.arrayItemRow}>
                                <input
                                    ...
                                    className={`${s.arrayInput} ${errors.education && !edu.trim()
                                        ? s.arrayInputError
                                        : s.arrayInputDefault
                                        }`}
